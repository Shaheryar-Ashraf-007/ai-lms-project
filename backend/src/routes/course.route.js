...
courseRouter.delete("/deleteLecture/:courseId/:lectureId", protectRoutes, deleteLecture);
courseRouter.get("/courselecture/:courseId", protectRoutes, getCourseLectures);
courseRouter.post("/editlecture/:lectureId", upload.single("video"), (err, req, res, next) => {
  if (err?.code === "LIMIT_FILE_SIZE") {
    return res.status(413).json({ message: "File too large. Max size is 700MB." });
  }
  next(err);
}, editLecture);
